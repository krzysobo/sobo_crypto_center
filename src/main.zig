...
fn encrypt(plain_text: []u8) !void {
    ...
    const key_size_bytes: comptime_int = sobocrypto_aes.PortableKeySuite.key_size_bytes;
    const nonce_size_bytes: comptime_int = sobocrypto_aes.PortableKeySuite.nonce_size_bytes;

    const allocator = std.heap.page_allocator;
    const buf_key = try allocator.alloc(u8, key_size_bytes);
    const nonce: []u8 = try allocator.alloc(u8, nonce_size_bytes);

    try sobocrypto_aes.makeRandomKey(buf_key, key_size_bytes);
    try sobocrypto_aes.makeNonce(nonce, nonce_size_bytes);

    const aad_text = "Dolor sit amet.";  // additional text for authentication
    ...
    try sobocrypto_aes.aesGcmEncrypt(encrypted_text, &aes_gcm_tag, plain_text, buf_key, nonce, aad_text, nonce_size_bytes, key_size_bytes);

    var pks = sobocrypto_aes.PortableKeySuite.init(buf_key, &aes_gcm_tag, nonce, aad_text);
    const buf_portable_key_hex_size = pks.getHexStringSize();
    const buf_portable_key_hex = try allocator.alloc(u8, buf_portable_key_hex_size);
    try pks.getHexString(buf_portable_key_hex);
    
    std.debug.print("ENCRYPTED TEXT:\n{s}\nPortable Key Suite:\n{s}\n\n", .{std.fmt.fmtSliceHexUpper(encrypted_text), buf_portable_key_hex});
}
